...
function surfaceToIndex(s) {
  ...
  var str = String(s);
  if (str.indexOf("+ 200") !== -1 || str.indexOf("151") !== -1) return 4;
...
function calculateDevisImmeuble(diagsActifs, answers, form) {
  ...
  // DTA PC si avant 1997
  if (annee < 1997) pcLignes.push("    • DTA parties communes : (voir grille interne)");
  // DPE collectif (immeuble monopropriété)
  pcLignes.push("    • DPE collectif : (voir grille interne)");
  // CREP PC si avant 1949
  if (annee < 1949) pcLignes.push("    • CREP parties communes : (voir grille interne)");
...
function DevisForm(props) {
  ...
  function handleGeneratePDF() {
    ...
    try {
      ...
      if (devis && devis.lignes.length > 0) {
        ...
        devisHtml += "</ul>";
        if (devis.totalHT !== null) devisHtml += "<p><strong>TOTAL HT : " + devis.totalHT.toFixed(2) + " €</strong></p>";
      }
    } catch (err) { devisHtml = ""; }
  ...
  function handleSend() {
    ...
    var templateParams = {
      ...
      diagnostics: (function() {
        ...
        try {
          var devis = calculateDevisHT(diagsActifs, answers, form);
          if (devis && (devis.lignes.length > 0 || devis.totalHT !== null)) {
            out += "\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
            ...
            devis.lignes.forEach(function(l) {
              if (l.sousLignes) {
                ...
                out += "  • " + l.label + " : " + (l.note || "sur devis") + "\n";
              }
            });
            if (devis.totalHT !== null) {
          ...
          out += "\n\n[Calcul prix non disponible — " + (e.message || "erreur") + "]";
        }
        return out;
      })(),
...
export default function DiagCheck() {
  ...
  var handleCode = useCallback(function() {
    ...
    if (code === "ASSISTANTE" || code === "ASSISTANT" || code === "MODELISA") {
      setLisaMode(true);
      setAgence({ nom: "Mode Assistante (Lisa)", email: MON_EMAIL });
