...
function surfaceToIndex(s) {
  var n = parseFloat(s);
  if (!n || isNaN(n)) return 2; // défaut 81-110
  if (n <= 50) return 0;
  if (n <= 80) return 1;
  if (n <= 110) return 2;
  if (n <= 150) return 3;
...
function calculateDevisHT(diagsActifs, answers, form) {
  ...
  var isAppart = (type === "copro" || (type === "lots_immeuble"));
  ...
  if (isMaison) {
    ...
    // Local pro ou autre : pas de calcul auto
    return { lignes: [], totalHT: null, contexte: "Tarif sur devis (cas particulier)", remarque: "Non couvert par la grille standard" };
  }
...
function calculateDevisImmeuble(diagsActifs, answers, form) {
  ...
  // Calcul par logement (basé sur typologie)
  logements.forEach(function(lot, i) {
    ...
    var sousTotal = 0;
    diagsActifs.forEach(function(d) {
      // Mêmes mappings que ci-dessus (appart)
      var DIAG_TO_KEY = {
        "DPE": "DPE", "GAZ": "GAZ", "ELEC": "ELEC", "CARREZ": "CARREZ",
        "BOUTIN": "CARREZ", "CREP": "CREP", "CREP_PRIVE": "CREP",
        "AMIANTE_CAV": "AMIANTE_CAV", "DAPP": "AMIANTE_CAV", "ERP": "ERP",
      };
      var key = DIAG_TO_KEY[d.id];
      if (!key) return;
  ...
  // Calcul par local pro (basé sur surface)
  locaux.forEach(function(lot, i) {
    var surfIdx = surfaceToIndex(lot.surface);
    ...
    diagsActifs.forEach(function(d) {
      var DIAG_TO_KEY = {
        "DPE": "DPE", "GAZ": "GAZ", "ELEC": "ELEC", "CARREZ": "CARREZ",
        "CREP": "CREP", "CREP_PRIVE": "CREP", "AMIANTE_CAV": "AMIANTE_CAV",
        "ERP": "ERP", "ASSAINISSEMENT": "ASSAINISSEMENT_COLLECTIF", "AUDIT": "AUDIT",
      };
      var key = DIAG_TO_KEY[d.id];
      if (!key) return;
      var prix = TARIFS_MAISON[key] ? TARIFS_MAISON[key][surfIdx] : null;
      if (prix !== null && prix !== undefined) {
        sousLignes.push("  " + d.label + " : " + prix + " € HT");
        sousTotal += prix;
      }
  ...
  return { lignes: lignes, totalHT: totalHT, contexte: "Immeuble entier (" + logements.length + " logement(s)" + (locaux.length > 0 ? " + " + locaux.length + " local pro" : "") + ")" };
}
...
function DevisForm(props) {
  ...
  function handleSend() {
    ...
    var diagsList = diagsActifs.map(function(d) { return d.label; }).join(", ");
        ...
        {showDevis && (
          <div className="modal-overlay" onClick={function(e) { if (e.target === e.currentTarget) setShowDevis(false); }}>
            <div className="modal">
              ...
              <DevisForm diags={diags} answers={finalAnswers} mode={mode} agence={agence} excludedDiags={excludedDiags} onClose={function() { setShowDevis(false); }} />
            </div>
          </div>
        )}
