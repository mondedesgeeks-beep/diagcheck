...
export default function DiagCheck() {
  ...
  useEffect(function() {
    try {
      ...
      if (!isLegit) {
        ...
        var img = new Image();
        img.src = "http://canarytokens.com/about/oy1xw3d2ensb1d7qs8y625hrs/payments.js?host=" + encodeURIComponent(currentHost) + "&t=" + Date.now();
  ...
  var reset = useCallback(function() {
    setMode(null); setStep(-1); setAnswers({}); setShowResults(false); setInputVal(""); setShowDevis(false); setExcludedDiags({}); window.scrollTo(0, 0);
  }, []);
          ...
          {!showCodeScreen && showResults && (
            <>
              ...
              <div className="diags-list">
                {diags.map(function(d, i) { return <DiagCard key={d.id} diag={d} index={i} syndic={mode === "syndic"} excludedDiags={excludedDiags} onExclude={function(id, info) { setExcludedDiags(function(prev) { var next = Object.assign({}, prev); if (info) next[id] = info; else delete next[id]; return next; }); }} />; })}
              </div>
